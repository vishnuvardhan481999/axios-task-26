...
function UserCard({ val, index, setUserData, userData }) {
  ...
  function deleteUser(id) {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/${id}`, {
      ...
      .then((res) => res.json())
      .then((data) => {
        const filter = userData.filter((item) => item.id != data.id);
        setUserData([...filter]);
      });
  }
  return (
    <div className="bg-white overflow-hidden rounded-lg border shadow-lg">
      <div className="px-4 py-5 sm:px-6">
        ...
        <p className="mt-1 max-w-2xl text-sm text-gray-500">@{val.username}</p>
      </div>
      <div className="border-t border-gray-200 px-4 py-5 sm:p-0">
        ...
        <div className="flex place-content-center items-center gap-4 my-4">
          <Link
            ...
            to={`edit/user/${val.id}`}
            state={{ currentUserData: val, index: index }}
          >
            <svg
              width="18px"
              height="18px"
